<template>

  <Header></Header>

  <section class="py-4">
    <div class="container hero4__top">
      <h2 class="flex flex-col	 hero__first__heading text-3xl md:text-6xl lg:text-6xl xl:text5xl mb-4">
        <span class="block md:inline">Top AI & ML</span>
        <span class="block md:inline">talent, active and&nbsp;ready</span>

      </h2>
    </div>

    <div class="container hero__technologies">
      <p class="hero__second__heading">Streamline your AI and ML search. Access our pool of 300,000 candidates.
      </p>
    </div>

    <section class="container mx-auto mt-4 mb-4">
      <div class="section_one_img_container">
        <img class="section_one_img_1"
          src="https://www.netsolutions.com/assets/images/cloud-computing-hub-banner-img-2.webp" alt="Landscape Image">
      </div>
    </section>

  </section>

  <section>
    <div class="container custom-container">
      <div class="row custom-row">
        <div class="custom-col">
          <h3>33% less</h3>
          <div>unpaid invoices</div>
        </div>
        <div class="custom-col">
          <h3>2x faster</h3>
          <div>payment collection</div>
        </div>
        <div class="custom-col">
          <h3>$170 million</h3>
          <div>payment processed</div>
        </div>
        <div class="custom-col">
          <h3>70k+ talent</h3>
          <div>made their life easier</div>
        </div>
      </div>
    </div>
  </section>

  <section>
    <div class="container">
      <div class="row">
        <div class="col-12 col-lg-6 mb-4">
          <a href="/technologies/business-applications" class="flex-link">
            <div class="flex-container">
              <div class="icon-container-holder">
                <svg width="32px" height="32px" viewBox="0 0 82 82" version="1.1" xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink">
                  <title>icon-applications</title>
                  <g id="Icons" stroke="none" stroke-width="1" fill="none" fill-rule="evenodd">
                    <g id="Desktop-HD" transform="translate(-942.000000, -748.000000)" fill="currentColor"
                      fill-rule="nonzero">
                      <g id="icon-applications" transform="translate(942.000000, 748.000000)">
                        <path
                          d="M35.582082,0 L2.41791805,0 C1.08306609,0 0,1.08331884 0,2.4184823 L0,35.5815177 C0,36.9166812 1.08306609,38 2.41791805,38 L35.582082,38 C36.9169339,38 38,36.9166812 38,35.5815177 L38,2.4184823 C38,1.08331884 36.9169339,0 35.582082,0 Z M33.1641639,33.1630354 L4.83583609,33.1630354 L4.83583609,4.83696459 L33.1641639,4.83696459 L33.1641639,33.1630354 Z"
                          id="Shape"></path>
                        <path
                          d="M79.5817229,0 L46.4182771,0 C45.0832269,0 44,1.08331884 44,2.4184823 L44,35.5815177 C44,36.9166812 45.0832269,38 46.4182771,38 L79.5817229,38 C80.9167731,38 82,36.9166812 82,35.5815177 L82,2.4184823 C82,1.08331884 80.9136293,0 79.5817229,0 L79.5817229,0 Z M77.1634458,33.1630354 L48.8365542,33.1630354 L48.8365542,4.83696459 L77.1634458,4.83696459 L77.1634458,33.1630354 Z"
                          id="Shape"></path>
                        <path
                          d="M35.582082,44 L2.41791805,44 C1.08306609,44 0,45.0833188 0,46.4184823 L0,79.5815177 C0,80.9166812 1.08306609,82 2.41791805,82 L35.582082,82 C36.9169339,82 38,80.9166812 38,79.5815177 L38,46.4184823 C38,45.0833188 36.9169339,44 35.582082,44 Z M33.1641639,77.1630354 L4.83583609,77.1630354 L4.83583609,48.8369646 L33.1641639,48.8369646 L33.1641639,77.1630354 Z"
                          id="Shape"></path>
                        <path
                          d="M79.5817229,44 L46.4182771,44 C45.0832269,44 44,45.0833188 44,46.4184823 L44,79.5815177 C44,80.9166812 45.0832269,82 46.4182771,82 L67.546764,82 C68.8818141,82 69.965041,80.9166812 69.965041,79.5815177 C69.965041,78.2463542 68.8818141,77.1630354 67.546764,77.1630354 L48.8365542,77.1630354 L48.8365542,48.8369646 L77.1634458,48.8369646 L77.1634458,79.5815177 C77.1634458,80.9166812 78.2466727,82 79.5817229,82 C80.9167731,82 82,80.9166812 82,79.5815177 L82,46.4184823 C82,45.0833188 80.9136293,44 79.5817229,44 L79.5817229,44 Z"
                          id="Path"></path>
                      </g>
                    </g>
                  </g>
                </svg>
              </div>
              <div class="text-container">
                <div class="header-holder-for-col">Business-appliccations</div>
                <div class="para-holder-for-col">
                  Cloud infrastructure offers scalability, allowing businesses to easily scale up or down their
                  resources (such as storage, computing power, and networking) based on demand.
                </div>
              </div>
            </div>
          </a>
        </div>

        <div class="col-12 col-lg-6 mb-4">
          <a href="/technologies/cloud-infrastructure" class="flex-link">
            <div class="flex-container">
              <div class="icon-container-holder">
                <svg width="31" height="30" viewBox="0 0 31 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M11.2891 3.08787C12.7391 1.27031 14.9379 0.117526 17.4571 0.0315393L17.7609 0.0263672C21.9 0.0263672 25.2622 2.99483 25.8655 6.76156L25.8761 6.83437L26.0017 6.88804C28.4159 7.96208 30.0554 10.2431 30.1586 12.8527L30.1638 13.1148C30.1638 16.9237 26.9261 20.004 22.9736 20.004H7.33531C3.27188 20.004 0.145142 17.2919 0.145142 13.7347C0.145142 11.171 1.75899 8.80189 4.14933 7.85673L4.21214 7.83337L4.23386 7.71895C4.5676 6.10128 5.4772 4.75938 6.78286 3.9354L6.99229 3.80963C8.23244 3.10168 9.70675 2.87404 11.1225 3.14223L11.2291 3.16437L11.2891 3.08787ZM23.9508 7.61773C23.7582 4.55871 21.1018 2.02637 17.7609 2.02637C15.5067 2.02637 13.5902 3.12443 12.4838 4.84981L12.0618 5.50779L11.3215 5.25725C10.1998 4.87771 8.96895 4.98418 7.98378 5.54657C6.9317 6.14711 6.23453 7.26654 6.11207 8.67778L6.051 9.38156L5.36727 9.5592C3.48136 10.0492 2.14514 11.8234 2.14514 13.7347C2.14514 16.1191 4.31822 18.004 7.33531 18.004H22.9736C25.8432 18.004 28.1638 15.7963 28.1638 13.1148C28.1638 11.0255 26.7393 9.18121 24.6426 8.50686L23.9936 8.29812L23.9508 7.61773Z"
                    fill="currentColor"></path>
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M8.13159 24.999C8.13159 23.3422 6.78845 21.999 5.13159 21.999C3.47474 21.999 2.13159 23.3422 2.13159 24.999C2.13159 26.6559 3.47474 27.999 5.13159 27.999C6.78845 27.999 8.13159 26.6559 8.13159 24.999ZM4.13159 24.999C4.13159 24.4467 4.57931 23.999 5.13159 23.999C5.68388 23.999 6.13159 24.4467 6.13159 24.999C6.13159 25.5513 5.68388 25.999 5.13159 25.999C4.57931 25.999 4.13159 25.5513 4.13159 24.999Z"
                    fill="currentColor"></path>
                  <rect x="14.1545" y="18.999" width="2" height="6.35132" rx="1" fill="currentColor"></rect>
                  <rect x="19.1316" y="18.999" width="2" height="7" rx="1" fill="currentColor"></rect>
                  <rect x="24.1316" y="23.999" width="2" height="5" rx="1" transform="rotate(90 24.1316 23.999)"
                    fill="currentColor"></rect>
                  <rect width="2" height="7" rx="1" transform="matrix(-1 0 0 1 11.1316 18.999)" fill="currentColor">
                  </rect>
                  <rect width="2" height="4" rx="1" transform="matrix(0 1 1 0 7.13159 23.999)" fill="currentColor">
                  </rect>
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M28.1316 24.999C28.1316 23.3422 26.7884 21.999 25.1316 21.999C23.4747 21.999 22.1316 23.3422 22.1316 24.999C22.1316 26.6559 23.4747 27.999 25.1316 27.999C26.7884 27.999 28.1316 26.6559 28.1316 24.999ZM24.1316 24.999C24.1316 24.4467 24.5793 23.999 25.1316 23.999C25.6839 23.999 26.1316 24.4467 26.1316 24.999C26.1316 25.5513 25.6839 25.999 25.1316 25.999C24.5793 25.999 24.1316 25.5513 24.1316 24.999Z"
                    fill="currentColor"></path>
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M18.1316 26.999C18.1316 25.3422 16.7884 23.999 15.1316 23.999C13.4747 23.999 12.1316 25.3422 12.1316 26.999C12.1316 28.6559 13.4747 29.999 15.1316 29.999C16.7884 29.999 18.1316 28.6559 18.1316 26.999ZM14.1316 26.999C14.1316 26.4467 14.5793 25.999 15.1316 25.999C15.6839 25.999 16.1316 26.4467 16.1316 26.999C16.1316 27.5513 15.6839 27.999 15.1316 27.999C14.5793 27.999 14.1316 27.5513 14.1316 26.999Z"
                    fill="currentColor"></path>
                </svg>
              </div>
              <div class="text-container">
                <div class="header-holder-for-col">Cloud & Infrastructure</div>
                <div class="para-holder-for-col">
                  Cloud infrastructure offers scalability, allowing businesses to easily scale up or down their
                  resources (such as storage, computing power, and networking) based on demand.
                </div>
              </div>
            </div>
          </a>
        </div>

        <div class="col-12 col-lg-6 mb-4">
          <a href="/technologies/cyber-security" class="flex-link">
            <div class="flex-container">
              <div class="icon-container-holder">
                <svg width="31" height="30" viewBox="0 0 31 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M29.4106 5.41933C29.297 2.95938 27.2155 0.999023 24.6644 0.999023H6.16666L5.94918 1.00379C3.42615 1.11462 1.41553 3.14406 1.41553 5.63138V17.3667L1.42042 17.5787C1.53409 20.0387 3.61557 21.999 6.16666 21.999H24.6644L24.8819 21.9943C27.4049 21.8834 29.4155 19.854 29.4155 17.3667V5.63138L29.4106 5.41933ZM6.16666 2.99902H24.6644L24.8396 3.00425C26.2871 3.09094 27.4155 4.24453 27.4155 5.63138V17.3667L27.4102 17.5319C27.3212 18.8988 26.1361 19.999 24.6644 19.999H6.16666L5.99148 19.9938C4.54399 19.9071 3.41553 18.7535 3.41553 17.3667V5.63138L3.42089 5.46613C3.50987 4.09928 4.69495 2.99902 6.16666 2.99902Z"
                    fill="currentColor"></path>
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M6.41553 27.1564C6.41553 27.0672 6.47565 26.9893 6.56187 26.9667L9.09482 26.3026C11.0513 25.7897 12.4155 24.0216 12.4155 21.999L18.4155 21.999C18.4155 24.0216 19.7798 25.7897 21.7362 26.3026L24.2692 26.9667C24.3554 26.9893 24.4155 27.0672 24.4155 27.1564V27.8735C24.4155 28.4951 23.9116 28.999 23.29 28.999H7.54102C6.91943 28.999 6.41553 28.4951 6.41553 27.8735V27.1564Z"
                    fill="currentColor"></path>
                  <path
                    d="M15.4155 5.99902C16.9506 5.99902 18.1655 7.21397 18.1655 8.74902C18.1655 9.57107 17.8067 10.2841 17.2417 10.7901C16.9554 11.0498 16.787 11.4347 16.7905 11.8213V15.624C16.7905 16.4096 16.2011 16.999 15.4155 16.999C14.63 16.999 14.0405 16.4096 14.0405 15.624V11.8213C14.0474 11.4348 13.8757 11.0498 13.5894 10.7901C13.0245 10.2841 12.6655 9.57107 12.6655 8.74902C12.6655 7.21397 13.8805 5.99902 15.4155 5.99902Z"
                    fill="currentColor"></path>
                </svg>
              </div>
              <div class="text-container">
                <div class="header-holder-for-col">Cyber&Security</div>
                <div class="para-holder-for-col">
                  Cloud infrastructure offers scalability, allowing businesses to easily scale up or down their
                  resources (such as storage, computing power, and networking) based on demand.
                </div>
              </div>
            </div>
          </a>
        </div>

        <div class="col-12 col-lg-6 mb-4">
          <a href="/technologies/data-analytics" class="flex-link">
            <div class="flex-container">
              <div class="icon-container-holder">
                <svg width="31" height="30" viewBox="0 0 31 30" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M29.2663 4.59403C29.1647 2.59149 27.5082 0.999023 25.4796 0.999023H5.06327L4.86816 1.00396C2.86479 1.10546 1.27167 2.7613 1.27167 4.78907V25.197L1.2766 25.392C1.37815 27.3946 3.03467 28.9871 5.06327 28.9871H25.4796L25.6747 28.9821C27.6781 28.8806 29.2712 27.2248 29.2712 25.197V4.78907L29.2663 4.59403ZM5.06327 2.99902H25.4796L25.6266 3.00496C26.5475 3.07965 27.2712 3.85006 27.2712 4.78907V25.197L27.2653 25.3438C27.1906 26.2636 26.4198 26.9871 25.4796 26.9871H5.06327L4.91631 26.9811C3.99542 26.9064 3.27167 26.136 3.27167 25.197V4.78907L3.27761 4.64228C3.35233 3.72248 4.1231 2.99902 5.06327 2.99902Z"
                    fill="currentColor"></path>
                  <rect x="14.2712" y="2.99902" width="2" height="24" fill="currentColor"></rect>
                  <rect x="27.2712" y="13.999" width="2" height="24" transform="rotate(90 27.2712 13.999)"
                    fill="currentColor"></rect>
                  <rect x="5.27124" y="7.45996" width="7" height="2" rx="1" fill="currentColor"></rect>
                  <rect x="18.2712" y="7.45996" width="7" height="2" rx="1" fill="currentColor"></rect>
                  <rect x="9.77124" y="4.99902" width="7" height="2" rx="1" transform="rotate(90 9.77124 4.99902)"
                    fill="currentColor"></rect>
                  <rect x="20.0314" y="18.2891" width="7" height="2" rx="1" transform="rotate(45 20.0314 18.2891)"
                    fill="currentColor"></rect>
                  <rect x="24.9532" y="19.7305" width="7" height="2" rx="1" transform="rotate(135 24.9532 19.7305)"
                    fill="currentColor"></rect>
                  <rect x="5.27124" y="18.999" width="7" height="2" rx="1" fill="currentColor"></rect>
                  <rect x="5.27124" y="21.999" width="7" height="2" rx="1" fill="currentColor"></rect>
                </svg>
              </div>
              <div class="text-container">
                <div class="header-holder-for-col">Data & Analytics</div>
                <div class="para-holder-for-col">
                  Cloud infrastructure offers scalability, allowing businesses to easily scale up or down their
                  resources (such as storage, computing power, and networking) based on demand.
                </div>
              </div>
            </div>
          </a>
        </div>

        <div class="col-12 col-lg-6 mb-4">
          <a href="/technologies/sap" class="flex-link">
            <div class="flex-container">
              <div class="icon-container-holder">
                <svg width="33" height="33" viewBox="0 0 33 33" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path
                    d="M27.7671 31.1565H5.06195C3.12997 31.1565 1.5481 29.574 1.5481 27.6239C1.5481 25.6733 3.13036 24.0898 5.06195 24.0898H27.7684C29.6996 24.0898 31.2809 25.6729 31.2809 27.6239C31.2809 29.5742 29.7002 31.1565 27.7671 31.1565Z"
                    stroke="currentColor" stroke-width="2"></path>
                  <path
                    d="M31.3861 28.3611C30.9566 28.3611 30.0856 28.0539 30.0143 27.6229L26.3879 4.95618C26.1656 3.61183 25.235 2.68952 23.9463 2.68952C23.9463 2.68952 18.9238 2.68952 16.4125 2.68952C13.9012 2.68952 8.87864 2.68952 8.87864 2.68952C7.59132 2.68952 6.70305 3.61318 6.47935 4.95618L2.81427 27.6229C2.73467 28.1025 1.77643 28.4232 1.29471 28.3489C0.810252 28.2719 0.480874 27.8193 0.559101 27.341L4.37714 4.33296C4.7422 2.14011 6.71984 0.422852 8.87864 0.422852H23.9463C26.1079 0.422852 28.0855 2.14011 28.4478 4.33431L32.2658 27.3423C32.3454 27.8206 32.0147 28.2719 31.5289 28.3503C31.4822 28.357 31.4342 28.3611 31.3861 28.3611Z"
                    fill="currentColor"></path>
                  <path
                    d="M27.4237 28.5146H5.4049C4.913 28.5146 4.51428 28.1152 4.51428 27.6225C4.51428 27.1298 4.913 26.7305 5.4049 26.7305H27.4237C27.9156 26.7305 28.3143 27.1298 28.3143 27.6225C28.3143 28.1152 27.9169 28.5146 27.4237 28.5146Z"
                    fill="currentColor"></path>
                </svg>
              </div>
              <div class="text-container">
                <div class="header-holder-for-col">SAP</div>
                <div class="para-holder-for-col">
                  Cloud infrastructure offers scalability, allowing businesses to easily scale up or down their
                  resources (such as storage, computing power, and networking) based on demand.
                </div>
              </div>
            </div>
          </a>
        </div>

        <div class="col-12 col-lg-6 mb-4">
          <a href="/technologies/software-development" class="flex-link">
            <div class="flex-container">
              <div class="icon-container-holder">
                <svg width="28" height="31" viewBox="0 0 28 31" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <path fill-rule="evenodd" clip-rule="evenodd"
                    d="M25.8318 7.01328L14.9478 0.730147C14.1953 0.29568 13.2681 0.29568 12.5156 0.730147L1.63162 7.01328C0.878616 7.44802 0.41497 8.25168 0.415528 9.12117V15.4732V17.2399V21.6874C0.41497 22.5569 0.878616 23.3606 1.63162 23.7953L12.5156 30.0784C13.2681 30.5129 14.1953 30.5129 14.9478 30.0784L25.8318 23.7953C26.5848 23.3606 27.0485 22.5569 27.0479 21.6874V15.6193C27.0604 15.538 27.0607 15.4546 27.0479 15.372V9.12117C27.0485 8.25168 26.5848 7.44802 25.8318 7.01328ZM25.4354 15.0252L25.8401 14.793C26.2285 14.5691 26.7249 14.7025 26.9488 15.0909C27.0004 15.1804 27.033 15.2757 27.0479 15.372V15.6193C27.0117 15.8544 26.873 16.0715 26.6508 16.1996L25.4329 16.8987L25.4354 15.0252ZM25.4329 16.8987L20.8539 19.5268V24.7966L25.0211 22.3887C25.2717 22.244 25.4261 21.9768 25.4264 21.6874L25.4329 16.8987ZM25.4354 15.0252L20.8539 17.6541V12.4614C20.858 12.3871 20.858 12.3127 20.8539 12.2384L25.4427 9.59139L25.4354 15.0252ZM2.03698 17.2399V16.8758L6.60948 19.5147V24.7966L2.44234 22.3887C2.19176 22.244 2.03728 21.9768 2.03698 21.6874V17.2399ZM2.03698 9.59139V15.0049L6.62569 17.6419V12.4614C6.62167 12.3871 6.62167 12.3127 6.62569 12.2384L2.03698 9.59139ZM14.1371 2.13676C13.8862 1.99194 13.5772 1.99194 13.3263 2.13676L9.15921 4.54056L13.7317 7.17948L18.3042 4.54056L14.1371 2.13676ZM7.43642 10.8318L2.84771 8.18478L7.53776 5.47695L12.1103 8.10776L7.62289 10.7061L7.43642 10.8318ZM12.921 14.9462V9.52248L8.43361 12.1127C8.30895 12.1847 8.2318 12.3174 8.23093 12.4614V17.646L12.921 14.9462ZM12.921 28.4367L8.23093 25.733V20.4632L12.7183 23.0413C12.7829 23.0807 12.8507 23.1146 12.921 23.1427V28.4367ZM9.04571 19.0526L13.529 21.6388C13.6544 21.7112 13.809 21.7112 13.9344 21.6388L18.4177 19.0526L13.7317 16.3488L9.04571 19.0526ZM19.2325 25.733L14.5424 28.4367V23.1427C14.6127 23.1146 14.6805 23.0807 14.7451 23.0413L19.2325 20.4511V25.733ZM14.5424 14.9462L19.2325 17.6257V12.4614C19.2316 12.3174 19.1545 12.1847 19.0298 12.1127L14.5424 9.52248V14.9462ZM19.8405 10.6859L15.3532 8.10776L19.9257 5.46884L24.6157 8.17667L20.027 10.8318L19.8405 10.6859Z"
                    fill="currentColor"></path>
                </svg>
              </div>
              <div class="text-container">
                <div class="header-holder-for-col">Software & Development</div>
                <div class="para-holder-for-col">
                  Cloud infrastructure offers scalability, allowing businesses to easily scale up or down their
                  resources (such as storage, computing power, and networking) based on demand.
                </div>
              </div>
            </div>
          </a>
        </div>

      </div>
    </div>
  </section>

  <Footer></Footer>

</template>

<script>
import Header from '~/components/Header.vue';
import Footer from "~/components/Footer.vue";
export default {
  components: {
    Header,
    Footer,
  },
};
</script>


<style scoped>
@import url('https://fonts.googleapis.com/css2?family=Manrope:wght@200..800&display=swap');


html {
  -webkit-text-size-adjust: 100%;
  -ms-text-size-adjust: 100%;
  font-family: "Manrope";
}

.section_one_img_1 {
  width: 100%;
  height: 100%;
  object-fit: cover;
}

.flex-container {
  display: flex;
  align-items: center;
  padding: 20px;
  border: 1px solid #000;
  border-radius: 8px;
}

.icon-container-holder {
  flex: 0 0 auto;
  width: 100px;
  height: 100px;
  background-color: #f6f8ff;
  display: flex;
  justify-content: center;
  align-items: center;
  border: 1px solid #101010;
  border-radius: 10px;
  box-shadow: 4px 4px #000;
  margin-right: 20px;
  color: #278867;
}

.text-container {
  flex: 1 1 auto;
}

.header-holder-for-col {
  font-size: 20px;
  font-weight: bold;
  line-height: 1.2;
  margin-bottom: 10px;
}

.para-holder-for-col {
  font-size: 15px;
}

.custom-container {
  max-width: 100%;
}

.custom-row {
  display: flex;
  flex-wrap: nowrap;
  justify-content: space-between;
}

.custom-col {
  flex: 1;
  max-width: 25%;
  box-sizing: border-box;
  text-align: center;
  padding: 10px;

}

.custom-col>h3,
.custom-col>div {
  text-align: center;
  margin: 0;
  line-height: 23px;
}

.flex-link {
  text-decoration: none;
  color: #051AD0;
}

.hero__first__heading {
  font-family: Manrope;
  color: #051AD0;
}

.flex-container:hover {
  border: none;
}

svg {
  color: #221E1E;
}

.section_one_img_container {
  width: 100%;
  height: 390px;
  display: flex;
  justify-content: center;
  align-items: center;
  border-radius: 13px;
  overflow: hidden;
}
</style>{}